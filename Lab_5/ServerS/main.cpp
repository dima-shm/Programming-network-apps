...
void main()
{
	...
	try
	{
		...
		cout << "Символическое имя сервера: " << hostname << endl << endl;
		...
		while (true) 
		{
			...
			clntInfo = gethostbyaddr((const char*)&clnt.sin_addr, 4, AF_INET);

			cout << "IP-адрес клиента:          " << inet_ntoa(clnt.sin_addr) << endl;
			cout << "Символическое имя клиента: " << clntInfo->h_name << endl;
